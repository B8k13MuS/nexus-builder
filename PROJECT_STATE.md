...
- Улучшить промпт критика, чтобы он не был слишком строгим для маленькой модели.
- Добавить логирование стоимости и времени выполнения в базу данных.
- Реализовать механизм повторных попыток (retry) для сетевых ошибок Telegram API.
- Добавить поддержку текстовых команд (не только голосовых).
